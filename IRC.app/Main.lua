...
local colorScheme = {
	...
	myMessageSender = 0x0F0F0F,
	myMessageText = 0x969696,
	otherMessageSender = 0x0F0F0F,
	otherMessageText = 0xFFFFFF,
	channelDataMessageSender = 0xFFFFFF,
...
local workspace, window = system.addWindow(GUI.filledWindow(1, 1, 110, 27, 0x202021))

local leftPanel = window:addChild(GUI.panel(1, 1, 21, 1, 0x0F0F0F))
...
local channelsText = leftLayout:addChild(GUI.text(1, 1, 0x202021, "  Channels"))
local channelsList = leftLayout:addChild(GUI.list(1, 1, leftPanel.width, 1, 1, 0, 0x0F0F0F, 0xFFFFFF, 0x0F0F0F, 0xFFFFFF, 0x3366CC, 0x202021, false))

local usersText = leftLayout:addChild(GUI.text(1, 1, 0x202021, "  Users"))
local usersList = leftLayout:addChild(GUI.list(1, 1, leftPanel.width, 1, 1, 0, 0x0F0F0F, 0xFFFFFF, 0x0F0F0F, 0xFFFFFF, 0x3366CC, 0x202021, false))

local systemText = leftLayout:addChild(GUI.text(1, 1, 0x202021, "  System"))
local systemList = leftLayout:addChild(GUI.list(1, 1, leftPanel.width, 1, 1, 0, 0x0F0F0F, 0xFFFFFF, 0x0F0F0F, 0xFFFFFF, 0x3366CC, 0x202021, false))
...
local contactAddButton = contactLayout:addChild(GUI.button(1, 1, contactButtonWidth, contactLayout.height, 0x3C3C3C, 0x000000, 0x878787, 0xB4B4B4, "+"))
local contactRemoveButton = contactLayout:addChild(GUI.button(1, 1, contactButtonWidth, contactLayout.height, 0x4B4B4B, 0x000000, 0x878787, 0xB4B4B4, "-"))
contactRemoveButton.colors.disabled.background = 0x4B4B4B
contactRemoveButton.colors.disabled.text = 0xFFFFFF
local settingsButton = contactLayout:addChild(GUI.button(1, 1, contactButtonWidth, contactLayout.height, 0x3C3C3C, 0x000000, 0x878787, 0xB4B4B4, "*"))
...
local chatInput = chatInputLayout:addChild(GUI.input(1, 1, 36, 3, 0x202021, 0xFFFFFF, 0x000000, 0x202021, 0x0F0F0F, "", "Type message here"))
...
local backgroundPanel = backgroundContainer:addChild(GUI.panel(1, 1, 1, 1, 0x0F0F0F))
...
local loginServerInput = loginServerLayout:addChild(GUI.input(1, 1, 27, 3, 0x202021, 0x787878, 0xB4B4B4, 0x202021, 0x0F0F0F, config.server or "", "Server"))
local loginPortInput = loginServerLayout:addChild(GUI.input(1, 1, loginServerLayout.width - loginServerInput.width - 1, 3, 0x202021, 0x787878, 0xB4B4B4, 0x202021, 0x0F0F0F, config.port and tostring(config.port) or "", "Port"))

local loginUsernameInput = GUI.input(1, 1, 36, 3, 0x202021, 0x787878, 0xB4B4B4, 0x202021, 0x0F0F0F, config.username or "", "Username")
local loginPasswordInput = GUI.input(1, 1, 36, 3, 0x202021, 0x787878, 0xB4B4B4, 0x202021, 0x0F0F0F, config.password or "", "Password")
local loginPasswordSwitchAndLabel = GUI.switchAndLabel(1, 1, 36, 8, 0x66DB80, 0x202021, 0xFFFFFF, 0xB4B4B4, "Log into NickServ:", false)
local loginSubmitButton = GUI.button(1, 1, 36, 3, 0xF2F2F2, 0xFFFFFF, 0x969696, 0xFFFFFF, "Connect")
local loginStatusText = GUI.text(1, 1, 0x000000, "")
...
local function contactItemDraw(item)
	...
	if not item.pressed and item.unreadCount > 0 then
		...
		screen.drawRectangle(item.x + item.width - tipWidth - 1, y, tipWidth, 1, 0x3C3C3C, 0x202021, " ")
		screen.drawText(item.x + item.width - tipWidth, y, 0x202021, tipString)
...
chat.draw = function()
	screen.drawRectangle(chat.x, chat.y, chat.width, chat.height, 0x0F0F0F, colorScheme.otherMessageText, " ")
	...
	if history[selectedItem.text] and #history[selectedItem.text] > 0 then
		...
		for i = chat.y, chat.y + chat.height - 1 do
			...
			screen.rawSet(index, background, 0x202021, "│")
		end
		...
		for i = scrollBar.value, 1, -1 do
			if messages[i].newMessages then
				...
				screen.drawText(chat.x + 1, y + 1, 0xF2F2F2, messages[i].time)
			end
	...
	local function addCategory(field, name)
		if #channelUsersList[field] > 0 then
			...
			for i = 1, #channelUsersList[field] do
				rightLayout:addChild(GUI.adaptiveButton(1, 1, 0, 0, nil, 0x969696, nil, 0x0F0F0F, channelUsersList[field][i])).onTouch = userButtonOnTouch
			end
		end
	end
...
settingsButton.onTouch = function()
	...
	local slider = backgroundLayout:addChild(GUI.slider(1, 1, 36, 0x66DB80, 0x202021, 0xFFFFFF, 0xB4B4B4, 0, 500, config.historyLimit, false, "History limit: ", ""))
	slider.height = 2
	slider.roundValues = true

	local switchAndLabel = backgroundLayout:addChild(GUI.switchAndLabel(1, 1, 36, 8, 0x66DB80, 0x202021, 0xFFFFFF, 0xB4B4B4, "Sound notifications:", config.soundNotifications))
	
	backgroundLayout:addChild(GUI.button(1, 1, 36, 3, 0xF2F2F2, 0xFFFFFF, 0x969696, 0xFFFFFF, "Save")).onTouch = function()
...
contactAddButton.onTouch = function()
	...
	local input = backgroundLayout:addChild(GUI.input(1, 1, 36, 3, 0x202021, 0x787878, 0xB4B4B4, 0x202021, 0x0F0F0F, "", "User or chat name"))
